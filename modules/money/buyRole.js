...
exports.exec = async (Bastion, message, args) => {
  try {
    ...
    if (buyableRoles.includes(role.id)) {
      let user = await Bastion.db.get(`SELECT lastmagicCurrency FROM profiles WHERE userID=${message.author.id}`);
      ...
      message.channel.send({
        embed: {
          ...
          description: `${message.author.tag} bought the **${role.name}** role for **${rolesInStore[role.id]}** LastMagic Currencies.`
        }
      }).catch(e => {
        Bastion.log.error(e);
      });
