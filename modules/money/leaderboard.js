...
exports.exec = async (Bastion, message, args) => {
  try {
    let profiles = await Bastion.db.all('SELECT userID, level, xp, lastmagicCurrencies FROM profiles ORDER BY level * 1 DESC, xp * 1 DESC, lastmagicCurrencies * 1 DESC');
    ...
    for (let i = 0; i < profiles.length; i++) {
      ...
      fields.push({
        name: `${i + 1 + (p * 10)}. ${user}`,
        value: `Level: ${profiles[i].level}\tExperience Points: ${profiles[i].xp}\tLastMagic Currencies: ${profiles[i].bastionCurrencies}`
      });
    }
