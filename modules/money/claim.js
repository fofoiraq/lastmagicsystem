  ...
  if (!claimedUsers.includes(message.author.id)) {
    ...
    if (Bastion.user.id === '267035345537728512') {
      if (message.guild.id === specialIDs.bastionGuild) {
        ...
        rewardMessage = `Your account has been debited with **${rewardAmount}** LastMagic Currencies.`;
      }
      else {
        rewardMessage = `Your account has been debited with **${rewardAmount}** LastMagic Currencies.\n\n`
          + '💡 **Pro Tip**\nYou can get **2x** more LastMagic Currencies when you use the `claim` or `daily`';
      }
    }
    else {
