...
exports.exec = async (Bastion, message, args) => {
  try {
    ...
    let userProfile = await Bastion.db.get(`SELECT lastmagicCurrency FROM profiles WHERE userID='${message.author.id}'`);
    ...
    message.channel.send({
      embed: {
        ...
        description: `${message.author.tag}, you successfully bought ${args.amount} ${gifts[args.product][0]} for ${requiredBalance} LastMagic Currencies.`
      }
    }).catch(e => {
      Bastion.log.error(e);
    });
